...
export const dragEnterEventFn = (ev: Event, node: TreeNode) => {
    ...
    if (!findParentPathHasEl(target, dragOriginEl.value)) {
        if (node.children) isTargetDir = true
        counter++
    ...
    // console.log('enter',dragTargetEl.value)
}
...
/*
watch(() => dragTargetEl.value, (value, oldValue) => {
    if (value === oldValue) return
    if (value)
        value.style.borderBottom = "2px solid blue"
    if (oldValue)
        oldValue.style.borderBottom = ""
})*/
...
watch(() => dragOverArea.value, (newValue, oldValue) => {
    if (dragTargetEl.value) {
        ...
        switch (newValue) {
            case DRAG_AREA.INSET:
                dragTargetEl.value.style.border = "1px solid black"
                break;
            case DRAG_AREA.TOP:
                dragTargetEl.value.style.borderTop = "3px double black"
                break;
            case DRAG_AREA.BOTTOM:
                dragTargetEl.value.style.borderBottom = "3px double black"
                break;
        }
    }
})
