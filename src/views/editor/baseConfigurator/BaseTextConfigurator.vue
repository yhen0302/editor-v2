...
import {
  ...
  FONT_VERTICAL_ALIGN,
  FONT_FAMILY
} from '@/views/editor/twoDimension/elements/constant'
...
export default {
  ...
  setup(props: any, context: SetupContext) {
    ...
    const fontFamily = computed<fontType>({
      get() {
        return editorGetter['GET_SELECT_NODE'].value.option.textOption
          .fontFamily
      },
      set(newVal) {
        editorGetter['GET_SELECT_NODE'].value.option.textOption.fontFamily =
          newVal
      }
    })
    ...
    const fontSize = computed({
      ...
      set(newVal) {
        editorGetter['GET_SELECT_NODE'].value.option.textOption.fontSize =
          newVal
      }
    })
    const fontStyle = computed({
      ...
      set(newVal) {
        editorGetter['GET_SELECT_NODE'].value.option.textOption.fontStyle =
          newVal
      }
    })
    ...
    const align = computed({
      ...
      set(newVal) {
        editorGetter['GET_SELECT_NODE'].value.option.textOption.align =
          newVal[0]
      }
    })
    const verticalAlign = computed({
      ...
      set(newVal) {
        ...
        editorGetter['GET_SELECT_NODE'].value.option.textOption
            .verticalAlign = newVal[0]
      }
    })
    return {
