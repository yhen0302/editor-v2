          ...
          <ul class="events-list flex flex-col">
            <li class="event-item justify-between flex" v-for="(item, index) in eventList" :key="index">
              ...
              <div class="event-edit-box flex">
                <img src="~@/assets/images/edit-event.png" class="event-edit-btn" height="16" @click="editEvent(index)" />
...
export default {
  ...
  setup() {
    ...
    const eventList = computed(() => getters2D.GET_SELECT_NODE?.value?.option.emitters || [])
    ...
    function editEvent(index) {
      ...
      effectVal.value  = node.value.option.emitters[index].effect
      echoIndex = index
      navIndex.value = 1
    }
